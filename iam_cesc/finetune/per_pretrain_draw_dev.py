...
def extract(
    ...
    if name.count("_") > 0:
        comment, checkpoint = [int(x) for x in name.rsplit("_", 2)[1:]]
...
if __name__ == "__main__":
    ...
    for metric, y_lim in metrics.items():
        ...
        axes: np.ndarray[Axes] = fig.subplots(len(modes), len(comments), sharex=True, sharey=True)
        max_x: int = 0

        for pos, mode in enumerate(modes.keys()):
            sub_data: list[tuple[str, int, int, np.ndarray, np.ndarray, list[np.ndarray]]] = sorted(
                [extract(name, content) for name, content in data.items() if name[0] == str(mode)],
                key=lambda x: x[0],
            )
        ...
        if max_x > 0:
            ...
            fig.suptitle(f"GreaseArg IAM CESC dev {metric}")
            fig.tight_layout()
            ...
            fig.savefig(summary_dir / f"dev_{metric}.png", dpi=300)
            print(f"dev_{metric}.png saved")
