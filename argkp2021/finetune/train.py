...
from model_finetune import GreaseArgModelForMultipleChoice
from torch.optim import AdamW
from torch.optim.lr_scheduler import LambdaLR
from trainer import ArgKP2021Trainer, compute_metrics
...
format_map: dict[str, ContextFormat] = {
    "s": ContextFormat.SPLIT,
    "p": ContextFormat.PAIR,
    "c": ContextFormat.CONCAT,
}
...
def get_optimizer(model: GreaseArgModelForMultipleChoice, args: TrainingArguments) -> AdamW:
...
def train(args: TrainArg, is_test: bool) -> None:
    ...
    name: str = f"{args.mode}_{args.method}"
    ...
    train_set, dev_set, test_set = ArgKP2021Dataset.load_train_dev_test(
        "argkp", format_map[args.method[0]]
    )
    ...
    use_cs: bool = args.method[1] == "c"
    model: GreaseArgModelForMultipleChoice
    ...
    if args.pretrain_dataset is not None:
        ...
        model = GreaseArgModelForMultipleChoice.from_pretrained(
            pretrain_dir, use_cs, input_text=input_text, input_graph=input_graph
        )
    else:
        ...
        model = GreaseArgModelForMultipleChoice(config, use_cs)
...
def parse_args() -> TrainArg:
    parser = ArgumentParser(
        description="Fine-tune GreaseArg model with " "ArgKP 2021 matching task."
    )

    parser.add_argument(
        "method",
        choices=tuple(f"{x}{y}" for x in format_map.keys() for y in ("l", "c")),
    ...
    parser.add_argument("-p", "--pretrain-dataset", type=str, help="Specify pretrain dataset")
