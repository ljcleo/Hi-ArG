...
if __name__ == "__main__":
    ...
    for metric, y_lim in metrics.items():
        ...
        fig: Figure = plt.figure(figsize=(len(formats) << 2, len(modes) * 3))
        axes: np.ndarray[Axes] = fig.subplots(len(modes), len(formats), sharex=True, sharey=True)
        ...
        for pos, format in enumerate(formats):
            ...
            for name, value in sub_data.items():
                ...
                target: list[tuple[list[int], list[float], list[float]]] = grouped[
                    int(components[0])
                ]
            ...
            for (mode, mode_name), ax in zip(modes.items(), axes[:, pos]):
                ax: Axes
                ...
                ax.set_ylim(y_lim)
                ax.grid(which="both")
                ax.set_title(f"{mode_name}, {format} format")
